...
async fn get_entries() -> Vec<Entry> {
    ...
    let request = Request::new_with_str_and_init("http://127.0.0.1:8080/database", &opts).unwrap();
    let window = gloo_utils::window();
    let resp_val = JsFuture::from(window.fetch_with_request(&request)).await.unwrap();
    let resp: Response = resp_val.dyn_into().unwrap();

    let entries = JsFuture::from(resp.text().unwrap()).await.unwrap();
